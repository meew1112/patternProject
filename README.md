...
### Colab setup
If you want to run the repo in Google Colab, use the following order in separate cells:
```python
...
For your own dataset, upload or mount a folder named `dataset` so the notebook sees:
```text
MedViTV2/
  dataset/
    train/
...
Then validate it before training:
```python
