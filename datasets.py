...
def build_dataset(args):
    ...
    if args.dataset == 'Custom':
        # Custom dataset from user's own data
        # Expects: data_dir with train/, val/, test/ folders and metadata.csv in root
        data_dir = args.dataset_dir if hasattr(args, 'dataset_dir') else './dataset'
        metadata_csv = os.path.join(data_dir, 'meta67.csv')
        ...
        # Load train dataset
        train_dataset = CustomDataset(data_dir, metadata_csv, split='train', transform=train_transform)
        # Load test dataset
        test_dataset = CustomDataset(data_dir, metadata_csv, split='test', transform=test_transform)
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, metadata_csv, split='train', transform=None):
        ...
        self.split = split.upper()  # Convert to uppercase to match CSV
    ...
    def __getitem__(self, idx):
        ...
        # Get image path - images are in split-specific folders
        img_path = os.path.join(self.data_dir, self.split.lower(), row['img_id'])
